# Contributing to OKF MCP Server
...
2. **Clone your fork:**
   ```bash
   git clone https://github.com/<your-username>/okf-mcp-server.git
   cd okf-mcp-server
   ```
...
1. **Create the tool module** in `okf_mcp_server/src/tools/`:
   ```python
   # okf_mcp_server/src/tools/your_tool.py
...
2. **Register in MCP server** (`okf_mcp_server/src/mcp.py`):
   ```python
...
4. **Update documentation** in `README.md` and `okf_mcp_server/src/tools/README.md`.
