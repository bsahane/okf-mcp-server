.PHONY: help install ingest-install sample clean test lint format coverage pre-commit local container deploy undeploy deps
...
ingest-install: ## Install the Docling ingestion extra into the venv
	@. .venv/bin/activate && uv pip install -e ".[dev,ingest]"

sample: ## Publish a snapshot from the synthetic sample corpus into ./data
	@. .venv/bin/activate && okf-ingest build samples/finance
...
coverage: ## Run tests with coverage report (80% minimum)
	...
	.venv/bin/python -m pytest --cov=okf_mcp_server --cov-report=term-missing --cov-report=html --cov-fail-under=80
...
deploy: ## Deploy to target (usage: make deploy openshift)
	@if [ "$(filter openshift,$(MAKECMDGOALS))" = "openshift" ]; then \
		...
		echo "  View logs: oc logs -l app=okf-mcp-server --tail=100"; \
		echo "  Get route: oc get route okf-mcp-server"; \
		echo "  Check status: oc get pods,svc,route -l app=okf-mcp-server"; \
	else \
