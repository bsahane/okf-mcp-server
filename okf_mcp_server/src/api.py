"""This module sets up the FastAPI application for the OKF MCP server.
...
from okf_mcp_server.src.oauth.service import OAuthService
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    try:
        if settings.ENABLE_AUTH:
            from okf_mcp_server.src.oauth.service import initialize_storage
    ...
    try:
        from okf_mcp_server.src.oauth.service import cleanup_storage
...
def _hostname(value: str) -> str:
    """Hostname part of a Host header or Origin URL, lower-cased."""
    parsed = urlparse(value if "://" in value else f"//{value}")
    return (parsed.hostname or "").lower()


class LoopbackOriginMiddleware(BaseHTTPMiddleware):
    """DNS-rebinding protection for an unauthenticated or loopback-bound server.

    FastMCP 2.14.2 does not enable the MCP SDK's Host/Origin validation, and
    the MCP transport spec requires servers to validate Origin. Only loopback
    Host headers and (if sent) loopback Origins are accepted, so a web page
    cannot reach the unauthenticated local server. It also applies inside a
    container bound to 0.0.0.0 while authentication is disabled.
    """

    async def dispatch(self, request: Request, call_next: Callable):
        """Reject requests whose Host or Origin is not loopback."""
        if request.url.path == "/health":
            # Health reveals no knowledge; keeps container/Kubernetes probes working.
            return await call_next(request)
        host = request.headers.get("host", "")
        origin = request.headers.get("origin")
        if _hostname(host) not in LOOPBACK_HOSTS:
            return Response("Invalid Host header", status_code=421)
        if origin is not None and _hostname(origin) not in LOOPBACK_HOSTS:
