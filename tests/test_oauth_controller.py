...
from okf_mcp_server.src.oauth import controller
...
class TestOAuthControllerHandleCallback:
    ...
    @patch("okf_mcp_server.src.oauth.controller.settings")
    @pytest.mark.asyncio
    async def test_handle_callback_success(self, mock_settings):
        ...
        with patch("okf_mcp_server.src.oauth.controller.OAuth2Handler") as mock_handler:
    ...
    async def test_handle_callback_local_development(self, mock_settings):
        ...
        with (
            patch("okf_mcp_server.src.oauth.controller.OAuth2Handler") as mock_handler,
            patch(
                "okf_mcp_server.src.oauth.controller.api_module", create=True
            ) as mock_api_module,
        ):
            mock_handler.get_access_token_from_authorization_code_flow.return_value = (
                mock_token
...
class TestOAuthControllerHandleToken:
    ...
    async def test_handle_token_authorization_code_grant(self, mock_settings):
        ...
        with patch(
            "okf_mcp_server.src.oauth.controller.verify_code_challenge",
            return_value=True,
        ):
...
class TestOAuthControllerHandleIntrospect:
    ...
    async def test_handle_introspect_active_token(self):
        ...
        with patch(
            "okf_mcp_server.src.oauth.controller.OAuth2Handler.introspect_token"
        ) as mock_introspect:
...
class TestAuthorizationCodeGrantEdgeCases:
    ...
    async def test_invalid_authorization_code(self):
        ...
        from okf_mcp_server.src.oauth.models import (
...
class TestRefreshTokenGrantEdgeCases:
    ...
    async def test_invalid_refresh_token(self):
        ...
        from okf_mcp_server.src.oauth.models import RefreshTokenRequest
