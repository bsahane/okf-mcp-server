...
from okf_mcp_server.src.api import app, get_host
...
class TestAPI:
    ...
    def test_health_endpoint(self):
        ...
        assert data["service"] == "okf-mcp-server"
    ...
    @patch("okf_mcp_server.src.api.settings")
    ...
    def test_app_imports(self):
        ...
        import okf_mcp_server.src.api

        assert okf_mcp_server.src.api.app is not None

    def test_server_initialization(self):
        """Test that the server is properly initialized."""
        # Arrange & Act
        from okf_mcp_server.src.api import server
...
class TestRegisterEndpointRoute:
    ...
    def test_register_endpoint_returns_model_dump(self):
        ...
        with (
            patch(
                "okf_mcp_server.src.oauth.routes.get_oauth_service",
                return_value=mock_oauth_service,
            ),
            patch(
                "okf_mcp_server.src.oauth.controller.handle_register",
...
class TestLoopbackOriginMiddleware:
    ...
    def test_loopback_host_and_origin_allowed(self):
        client = TestClient(app, base_url="http://127.0.0.1:5001")
    ...
    def test_foreign_origin_rejected(self):
        ...
        for origin in ("https://attacker.example", "null"):
            response = client.get("/mcp", headers={"Origin": origin})
            assert response.status_code == 403

    def test_health_stays_reachable_for_probes(self):
        client = TestClient(app, base_url="http://10.0.0.7:5001")
        assert client.get("/health").status_code == 200

    def test_mcp_endpoint_is_protected(self):
        client = TestClient(app, base_url="http://attacker.example")
        response = client.post(
            "/mcp",
