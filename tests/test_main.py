...
from okf_mcp_server.src.main import (
...
class TestValidateConfig:
    ...
    @patch("okf_mcp_server.src.main.validate_config_func")
    @patch("okf_mcp_server.src.main.logger")
...
class TestHandleStartupError:
    ...
    @patch("okf_mcp_server.src.main.logger")
    @patch("okf_mcp_server.src.main.sys")
...
class TestMain:
    ...
    @patch("okf_mcp_server.src.main.validate_config")
    @patch("okf_mcp_server.src.main.logger")
    @patch("okf_mcp_server.src.main.uvicorn")
    def test_main_success(self, mock_uvicorn, mock_logger, mock_validate):
        ...
        with patch("okf_mcp_server.src.main.settings", mock_settings):
            # Act
            main()
    ...
    @patch("okf_mcp_server.src.main.run")
    @patch("okf_mcp_server.src.main.logger")
    @patch("okf_mcp_server.src.main.sys")
    def test_main_keyboard_interrupt(self, mock_sys, mock_logger, mock_run):
        ...
        # Assert
        mock_logger.info.assert_called_with("OKF MCP server shutting down")
        # The actual implementation might exit with 1 instead of 0
        mock_sys.exit.assert_called_with(1)

    @patch("okf_mcp_server.src.main.validate_config")
    @patch("okf_mcp_server.src.main.handle_startup_error")
...
class TestRun:
    ...
    @patch("okf_mcp_server.src.main.main")
    @patch("okf_mcp_server.src.main.logger")
    @patch("okf_mcp_server.src.main.sys")
