...
6. **Run the server:**
   ...
   python -m okf_mcp_server.src.main

   # Or using uv to run directly
   uv run python -m okf_mcp_server.src.main
   ```
...
2. **Test MCP tools:**
   ...
   ```bash
   # Test the search tool via MCP endpoint
   curl -X POST "http://localhost:5001/mcp" \
        -H "Content-Type: application/json" \
        -d '{"method": "tools/call", "params": {"name": "search_knowledge", "arguments": {"query": "hotel limit"}}}'
   ```
...
2. **Run tests with coverage reporting:**
   ...
   ```bash
   pytest --cov=okf_mcp_server --cov-report=html --cov-report=term
   ```
...
- `test_tools.py` - Knowledge tool tests (browse, search, get; traversal, bounds, isError)
- `test_ingest.py` - Ingestion, snapshots, validation and evaluation
- `test_docling_adapter.py` - Docling adapter (`pytest -m docling`, needs the ingest extra)
...
3. **Docstring validation:**
   ...
   ```bash
   pydocstyle okf_mcp_server/ --convention=google
   ```
