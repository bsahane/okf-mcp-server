...
> **Note:** The `greet_user` tool in this tutorial is a hands-on exercise for you to build. It is **not** included in the template — this project ships with `browse_knowledge`, `search_knowledge` and `get_knowledge`. New tools should raise `fastmcp.exceptions.ToolError` on failure so clients receive `isError: true`. Follow along to learn the pattern, then delete or keep it as you wish.
...
Create `okf_mcp_server/src/tools/greet_tool.py`:

```python
"""Greet tool for the OKF MCP Server.
...
Open `okf_mcp_server/src/mcp.py` and make two edits.

**Add the import** (with the other tool imports near the top):

```python
from okf_mcp_server.src.tools.greet_tool import greet_user
```
...
1. **Tool file** -- one function per file in `okf_mcp_server/src/tools/`.
...
| Tool documentation format reference | [Tools README](../okf_mcp_server/src/tools/README.md) |
