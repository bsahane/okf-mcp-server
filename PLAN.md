...
- **Semantic search (24 September 2026):** hybrid retrieval fuses BM25 and vector rankings with reciprocal-rank fusion (k = 60), after which sections are de-duplicated and deprecated concepts demoted. Embeddings come from `intfloat/multilingual-e5-small` (MIT, 94 languages, 384 dimensions), pinned to revision `614241f6` and loaded only from the local Hugging Face cache (`okf-ingest fetch-model`, 493 MB); the multilingual model was chosen because the corpora already contain German, Korean and Chinese text. Vectors live in the snapshot's SQLite index and are compared by brute force with numpy; rebuilds reuse vectors by text hash and load the model only when something needs embedding (no-op rebuild 0.47 s). Embedding runs at 69 passages/s on the Apple GPU (MPS) and 17/s on CPU, with identical vectors (max difference 2e-7). Measured with `okf-ingest eval`: pilot papers Hit@1 0.44 → 0.56, Hit@5 0.81 → 0.94, MRR 0.57 → 0.70, all-evidence 0.69 → 0.81; Northwind Hit@1 0.81 → 0.97, MRR 0.89 → 0.98, Hit@5 and all-evidence unchanged. The natural-language questions that failed earlier ("What attention does Kimi K3 use?", "doctor's note", "sign off a 25,000 pound purchase") now rank the right section first. Found while testing live: FastMCP 2.14 runs synchronous tools on the event loop, so loading the model inside the first request stalled every connection and hung a client. The server now loads the model at startup in a worker thread; a query then takes about 20 ms (10 concurrent searches in 643 ms). Without torch/transformers or the cached model, search falls back to keyword-only, which is what the server container and the main CI job run.
- **Remaining roadmap (25 September 2026):**
  - *Phase 1 on a real corpus:* the GitLab Handbook's People Group and Total Rewards sections (94 pages, 2,069 passages, public), 33 questions written from page titles only: hybrid Hit@1 0.73, Hit@5 0.93, MRR 0.83, all-evidence 0.83 (keyword 0.60 / 0.80 / 0.67 / 0.63). It exposed that Hugo frontmatter titles were discarded; they are now kept. The decisive pilot on the organization's own documents and user questions remains.
  - *Phase 5, access control:* the auth middleware turns validated introspection claims into an identity (issuer, subject, email, groups) on the request scope, with audience/scope checks (403). Documents carry `access` principals from `_okf.yaml`, connector-recorded source permissions or prefix rules; unlabelled documents are denied by default. Search filters keyword and vector candidates before ranking, browse hides empty folders, denied fetches read as not found, and every call is audited. Verified against a real Keycloak 26.4 (12/12): 401 without token, 403 for another client's token, per-group visibility, revocation, group removal, audit.
  - *Vectors at scale:* numpy needs ~3.6 KB/vector of memory (300k: 1.1 GB, 32 ms), sqlite-vec ~40 MB flat (300k: 180 ms). Builds pick numpy up to 50k vectors and sqlite-vec above. Filtering now happens before ranking (previously a restricted user could get nothing from a large corpus); the allowed-ID scan (~200 ms at 300k) is cached per snapshot.
